...
def clustering_car_pw(n, k, dis_matrix, ml, cl, lower, upper, exclusion_detail):
...
def read_input(folder_name, is_dcc=False):
    ...
    dataset_folder = "/home/henry/codes/deep_constrained_clustering/experiments/generating-constraints/" + args.data + "-numclass-dis/"
    with open(os.path.join(dataset_folder, args.data + "-6000/test00/numclass-feature.json"), 'r') as f:
        exclusion_detail = json.load(f)
    return n, k, p, ml, cl, label, exclusion_detail
...
def run_model(folder_name, is_dcc):
    n, k, p, ml, cl, label, exclusion_detail = read_input(folder_name, is_dcc)
    counter = dict()
    for l in label:
        if l in counter:
            counter[l] += 1
    ...
    model = clustering_car_pw(n, k, p, ml, cl, min_car, max_car, exclusion_detail[:10])
...
def run_experiment(dataset_folder, is_dcc):
    ...
    for pairwise_factor in PW_ARR:
        ...
        for testid in range(TOTAL_TEST_FOR_EACH_SET):
            ...
            if is_dcc:
                # if os.path.exists(test_folder + "/dcc-all"):
                #     continue
                if not os.path.exists(test_folder + "/dcc-all"):
                    os.makedirs(test_folder + "/dcc-all")
                np.savetxt(test_folder + "/dcc-all/pred.txt", pred, fmt='%s')
                local_stat = np.asarray([time_running, acc, nmi])
                np.savetxt(test_folder + "/dcc-all/stat.txt", local_stat, fmt='%s')
                continue
            # if os.path.exists(test_folder + "/all"):
            #     continue
            if not os.path.exists(test_folder + "/all"):
                os.makedirs(test_folder + "/all")
            np.savetxt(test_folder + "/all/pred.txt", pred, fmt='%s')
            local_stat = np.asarray([time_running, acc, nmi])
            np.savetxt(test_folder + "/all/stat.txt", local_stat, fmt='%s')
    return stats
