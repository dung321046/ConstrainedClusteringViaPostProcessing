...
def read_input(folder_name, is_dcc=False):
    ...
    if is_dcc:
        ...
        p = np.loadtxt(
            "/home/henry/codes/deep_constrained_clustering/experiments/generating-constraints/qdisFashion.txt")
    ...
    for pairwise_factor in PW_ARR:
        ...
        for testid in range(TOTAL_TEST_FOR_EACH_SET):
            ...
            if not is_dcc:
                continue
            # if is_dcc -> /post-dcc
            # if not is_dcc (idec) -> /post
            if os.path.exists(test_folder + "/post-dcc"):
                continue
            os.makedirs(test_folder + "/post-dcc")
            np.savetxt(test_folder + "/post-dcc/pred.txt", pred, fmt='%s')
            local_stat = np.asarray([time_running, acc, nmi])
            np.savetxt(test_folder + "/post-dcc/stat.txt", local_stat, fmt='%s')
    return stats
