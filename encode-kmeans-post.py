...
if __name__ == "__main__":
    ...
    for seed in range(5):
        kmeans = KMeans(k, n_init=20, random_state=seed)
        y_pred = kmeans.fit_predict(X)
        folder_name = "/home/henry/codes/deep_constrained_clustering/experiments/generating-constraints/" + args.data + "-pw/"
        np.savetxt(folder_name + "encode-kmeans-" + str(seed), y_pred, fmt="%s")
    '''
    ...
    for pairwise_factor in [6, 50, 100]:
        ...
        for test in range(5):
            ...
            if os.path.exists(test_folder + "/encode-kmeans-post"):
                continue
            os.makedirs(test_folder + "/encode-kmeans-post")
