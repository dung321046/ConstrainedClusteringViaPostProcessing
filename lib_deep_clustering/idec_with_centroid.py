...
class IDEC_with_init_pred(nn.Module):
    ...
    def fit(self, stat, y_pred, ml_list, cl_list, ml_ind1, ml_ind2, cl_ind1, cl_ind2, ml_p, cl_p, X,
            y=None, lr=0.001, batch_size=256, num_epochs=100, update_interval=1, tol=1e-3):
        '''X: tensor data'''
        # use_cuda = torch.cuda.is_available()
        # if use_cuda:
        #     self.cuda()
